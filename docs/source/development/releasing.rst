...
<https://keepachangelog.com/>`_; the current line is **v0.1.3** (CPU-only;
...
:file:`docs/source/conf.py`                   Reads the installed metadata; ``0.1.3`` fallback.
...
.. note::
   ...
   * The **package version** (``0.1.3``) — the release identity described above.
   ...
   * The **config-schema version** (``config_version: "0.1.3"`` in the config
