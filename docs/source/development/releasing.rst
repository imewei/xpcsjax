...
<https://keepachangelog.com/>`_; the current line is **v0.1.0** (CPU-only;
...
:file:`docs/source/conf.py`                   Reads the installed metadata; ``0.1.0`` fallback.
...
.. note::
   ...
   * The **package version** (``0.1.0``) — the release identity described above.
   ...
   * The **config-schema version** (``config_version: "0.1.0"`` in the config
     templates) — versions the YAML config format. It matches the package
     version today only by coincidence.
