...
<https://keepachangelog.com/>`_; the current line is **v0.1.2** (CPU-only;
...
:file:`docs/source/conf.py`                   Reads the installed metadata; ``0.1.2`` fallback.
...
.. note::
   ...
   * The **package version** (``0.1.2``) — the release identity described above.
