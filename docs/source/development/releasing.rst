...
<https://keepachangelog.com/>`_; the current line is **v0.1.8** (CPU-only;
...
:file:`docs/source/conf.py`                   Reads the installed metadata; ``0.1.8`` fallback.
...
.. note::
   ...
   * The **package version** (``0.1.8``) — the release identity described above.
   ...
   * The **config-schema version** (``config_version: "0.1.8"`` in the config
