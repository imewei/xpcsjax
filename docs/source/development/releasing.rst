...
<https://keepachangelog.com/>`_; the current line is **v0.1.4** (CPU-only;
...
:file:`docs/source/conf.py`                   Reads the installed metadata; ``0.1.4`` fallback.
...
.. note::
   ...
   * The **package version** (``0.1.4``) — the release identity described above.
   ...
   * The **config-schema version** (``config_version: "0.1.4"`` in the config
