...
- [ ] **Step 2b: Confirm per-tier coverage — name which tiers have real end-to-end proof and which don't**

**Round-4 CCG dual-model review correction (codex finding #11):** v4's "best-effort... document rather than silently skip" language for hybrid-streaming/stratified-LS/sequential/out-of-core no longer describes this plan — Tasks 7c (out-of-core), 7d (sequential), 7e (stratified-LS), 7f (hybrid-streaming) each carry their own strip/restore mechanism AND their own dedicated proof test, but the tiers do **not** all have the same strength of proof. Do not treat this step as a uniform "confirmation pass" — state explicitly, per tier, what the test actually proves:

- **out-of-core (7c):** real end-to-end solve, `test_fixed_parameter_survives_out_of_core_fit` forces the branch via a monkeypatched `select_nlsq_strategy` and asserts on the real fitted result. Real proof. One caveat: the `out_of_core_recheck_delegation` second call site is wired identically but has no independent test (byte-identical code, reviewer-verified low risk).
- **sequential (7d):** **not a real solve.** A genuine, pre-existing bug in `sequential.py`'s own `has_fixed` residual-wrapping closure (`restore_fixed_parameters` called on a JAX tracer mid-JIT-trace, `parameter_utils.py:213`) crashes any real fit through this tier whenever any physical parameter is fixed (this is not scoped to partial fixes — there is no all-fixed short-circuit; see the ledger's Task 7d ruling). `test_fixed_parameter_wiring_reaches_sequential_solver` stubs the solver and asserts on what `wrapper.py` passes to it — this proves the *wiring* is correct, not that a fixed-parameter sequential fit can complete. This is a real, out-of-plan-scope functional gap, not a documentation nit — report it as such in this step's commit message, don't let the passing test read as "sequential tier works."
- **stratified-LS (7e) / hybrid-streaming (7f):** status depends on what Step 0 of each task found — verify at this point whether their proof tests are real solves or stubs (7d's JIT-tracer hazard is a known risk for any tier that restores mid-JIT-trace; 7e/7f's dispatches were told to check this explicitly in their own Step 0). Name the actual proof strength for each here, don't assume parity with 7c.

Run the full set of per-tier tests together and confirm none regress each other (they share the same `_synthetic_data`/`_config` helpers with different `extra_top`/size overrides):
