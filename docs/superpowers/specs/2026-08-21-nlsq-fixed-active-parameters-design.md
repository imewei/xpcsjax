...
| Q10 (post-implementation, 2026-08-22) | dev-suite:three-brain's comprehensive plan/spec review flagged Q3's "preserve sequential's existing zero-covariance convention" as apparently unreachable: `fit_nlsq_jax`'s top-level `resolve_optimized_physical_parameters` call always runs with `allow_all_fixed=False` (its default) and rejects an all-physical-fixed config with `ValueError` before tier selection ever runs — including when `force_sequential_fallback` would route to sequential. Should `allow_all_fixed=True` be threaded through to unblock this, e.g. by adding a homodyne-side broad (physics+scaling) zero-varying guard mirroring Q9's heterodyne one? | **No — the premise was already false, not a regression.** Verified directly: `sequential.py`'s `has_fixed` branch (pre-existing code, unrelated to this plan) has no all-fixed short-circuit of its own, and before this plan's resolver existed nothing threaded `fixed_parameters` into that tier at all — so there was no *tested*, publicly-reachable "existing convention" for Q3 to preserve; Q3's wording assumed reachability that empirical testing (an all-physical-fixed `fit_nlsq_jax` call, `force_sequential_fallback: true`) disproves. The current `ValueError` is also strictly better UX than a silent zero-length-covariance result for a zero-physical-DOF config. Loosening the gate would additionally remove the only safety net across the other six tiers (CMA-ES, multistart, out-of-core, stratified-LS, hybrid-streaming, plain) — none of which have sequential's fast path, unlike heterodyne where Q9's guard was purpose-built for the paths that actually reach it. `allow_all_fixed` remains an internal-only escape hatch with no current caller; this is intentional, not a loose end. |
...
Design section below reflects resolutions Q1-Q9; Q10 is a post-implementation
addendum recorded after the design's Component work landed.
