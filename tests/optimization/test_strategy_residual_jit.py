...
def test_dt_none_uses_fallback_and_warns() -> None:
    ...
    chunk.dt = None
    rf = StratifiedResidualFunctionJIT(
        _stratified([chunk]), per_angle_scaling=False, physical_param_names=_PHYSICAL
    )
...
def test_padding_zeros_masked_in_output() -> None:
    ...
    rf = StratifiedResidualFunctionJIT(
        _stratified([chunk_a, chunk_b]), per_angle_scaling=False, physical_param_names=_PHYSICAL
    )
...
def test_inconsistent_q_raises() -> None:
    ...
    with pytest.raises(ValueError, match="Inconsistent q values"):
        StratifiedResidualFunctionJIT(
            _stratified([a, b]), per_angle_scaling=False, physical_param_names=_PHYSICAL
        )
