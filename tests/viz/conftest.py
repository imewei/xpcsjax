...
def converged_heterodyne_result(synthetic_multi_angle_data):
    ...
    Canonical scaling-first layout (Tasks 3-12 of per-angle-mode unification):
        [contrast_0..n_phi-1, offset_0..n_phi-1, physical_0..13]
    Total: 2 * n_phi + 14 entries.

    A regression that reads scaling as physics would map small contrast/offset
    values (~0.2 / 1.0) into the physics slots and collapse the fitted c2
