...
def test_velocity_integral_constant_velocity_is_linear_signed_gap():
    ...
    m = np.asarray(
        compute_velocity_integral_matrix(_T, v0=0.0, beta=1.0, v_offset=_C, dt=_DT)
    )
...
def test_velocity_integral_is_antisymmetric_with_zero_diagonal():
    m = np.asarray(
        compute_velocity_integral_matrix(_T, v0=1.0, beta=1.0, v_offset=0.3, dt=_DT)
    )
...
def test_transport_integral_constant_rate_is_abs_gap_and_symmetric():
    ...
    m = np.asarray(
        compute_transport_integral_matrix(_T, D0=0.0, alpha=1.0, offset=_C, dt=_DT)
    )
