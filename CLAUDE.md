...
| L2 | Hierarchical Optimization | `hierarchical.py` | `individual` mode only |
...
Layer gating has two orthogonal axes: **model-lineage gating** (via `_LAYER_GATES` in `anti_degeneracy_controller.py`) and **per-angle mode gating** (via `per_angle_mode_actual` resolution). Layers absent from `_LAYER_GATES` are default-active for all analysis modes; only L5 is model-lineage-gated. L5 up-weights data near the flow direction φ0 to exploit the shear-sensitivity peak, which exists only when the kernel has a shear rate — so L5 is active for `laminar_flow` **only**. The static modes (`static_anisotropic`, `static_isotropic`) have no flow direction and `two_component` (heterodyne) has no shear rate, so L5 short-circuits for all of them.

**L2's per-angle mode gate (2026-08-09, #50):** The `HierarchicalOptimizer` class is *constructed* only when `per_angle_mode_actual == "individual"` (see `anti_degeneracy_controller.py:465`). For `averaged` and `constant` modes, the L2 setup banner fires when `hierarchical_enable` is configured (to maintain the "configured-not-executed" log parity contract), but `self.hierarchical` stays `None` and the `.fit()` call is never safe. Callers must check `use_hierarchical` or `l2_applicable` before invoking L2. The heterodyne hybrid-streaming path documents an equivalent gate at "L2 (`HierarchicalOptimizer`) gated to `individual`/`fourier`" below; the laminar standard in-memory path runs no L2 at all regardless (see the "Gap A is NOT a parity gap" note further down).

Note `is_layer_active()` still returns `True` for every layer when `analysis_mode=None` (the homodyne characterization gate's path), so model-lineage gating does not affect the rtol=1e-10 parity baselines.

**L2's `jaxopt` dependency is unmaintained upstream; no migration timeline is set.** L2's bounded L-BFGS warm-start (`hierarchical.py`) uses `jaxopt`, which upstream no longer maintains — its `DeprecationWarning` is suppressed both at the import site and globally in `pyproject.toml`'s pytest config. The planned replacement is `optimistix`, which **is now present in `uv.lock` as a transitive dependency** (verified 2026-09-04: `nlsq>=0.7.4` declares `optimistix>=0.1.0`, pulling `equinox`/`lineax`/`jaxtyping`/`wadler-lindig` with it, so `import optimistix` succeeds). It is **not** a direct xpcsjax dependency — a migration would still need to declare it in `pyproject.toml` rather than rely on nlsq continuing to pull it. Don't confuse this with `core.py`'s module docstring, which records that xpcsjax's *main NLSQ solve* previously migrated **away from** Optimistix to the NLSQ package; that was a separate, unrelated migration, so reintroducing `optimistix` here (scoped to L2's warm-start only) is a new addition, not a revert. No PR or issue tracks this jaxopt→optimistix migration yet; it is an accepted, undated v0.1 carry-over, not scheduled work.
