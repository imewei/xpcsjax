...
- **100k–1 M shuffle regime (laminar-parity).** Homodyne reorganizes + seed-42 pre-shuffles ≥100k-point per-angle fits that stay in-memory (stratified-LS engages only at ≥1 M). The heterodyne in-memory path mirrors this via an **objective-invariant seed-42 angle-axis reorder** in `_fit_nlsq_heterodyne` (gated `use_strat and 100_000 ≤ n_points < 1_000_000 and not cmaes`), since heterodyne data is batched by angle (`(n_phi, N, N)`) — the angle axis is the stratification unit, not a flat point list. **Scoped to `averaged` mode ONLY** (the `auto` default at `n_phi ≥ 3`): it collapses scaling to two GLOBAL params, so the fit *objective* is angle-order-invariant and only diagnostics are angle-indexed (realigned to the caller's order via `_restore_angle_order`, which inverse-permutes *every* `*_per_angle*` array). The fitted vector matches up to FP summation; at a degenerate minimum equi-objective param sets may still differ (same as laminar's shuffle). **`constant` is intentionally NOT reordered** — it freezes PER-ANGLE quantile scaling and writes `model.scaling` in angle order, so reordering would leak shuffled per-angle state; `individual`/`fourier` keep an angle-ordered param+covariance tail. Only 3-D `(n_phi,N,N)` weights are permuted; shared 2-D `(N,N)` weights pass through. Parity guard: `tests/heterodyne/test_shuffle_regime_parity.py`.
