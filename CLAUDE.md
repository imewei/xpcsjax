...
- **Two real-data oracles are AVAILABILITY-gated, not env-gated.** `tests/heterodyne/test_two_component_real_data.py::test_heterodyne_multi_angle_matches_source` and `tests/parity/test_l4_per_iteration_parity.py::test_homodyne_characterization_bit_identical_with_monitor` RUN automatically whenever their prerequisites are present (the C044 dataset/baseline for the first; the upstream `homodyne` package + the `Simon/`+`C020/` configs for the second) and SKIP with a clear reason otherwise. So on a maintainer machine they run under a plain `make test` (no env var, no skip); on CI / fresh clones they skip gracefully and **never fail**. Do **not** re-gate them on `XPCSJAX_RUN_CHARACTERIZATION` — that env-var-in-CI footgun is exactly what the availability check (`importlib.util.find_spec("homodyne")` + dataset-path existence) replaces.
...
- **The homodyne characterization suite is a maintainer-local LIVE oracle.** `tests/characterization/test_homodyne_equivalence.py` runs **only** when `XPCSJAX_RUN_CHARACTERIZATION=1`; otherwise it self-skips. The L4 bit-identity wrapper `tests/parity/test_l4_per_iteration_parity.py::test_homodyne_characterization_bit_identical_with_monitor` is now **AVAILABILITY-gated** (no longer env-gated): it runs that suite in a subprocess with `XPCSJAX_RUN_CHARACTERIZATION=1` **forced in the child env** (so the child does not self-skip and the bit-identity assertion is not vacuous), and skips at the parent level only when the upstream `homodyne` package or the `Simon/`+`C020/` configs are absent — so it executes on a maintainer machine without any env var and stays green (skipped) on CI. **(Phase 5, 2026-06-12)** The child subprocess is scoped via `-k "test_homodyne_bit_equivalence and laminar"` to the single laminar `rtol=1e-10` bit-identity node (now exercising EXPLICIT `per_angle_mode="individual"`, DG-A): that is the per-angle-mode permutation tripwire (static has no per-angle flip), and isolating the one ~23M-point fit keeps the child within its memory budget — the `≥23 M` laminar fit needs ~61 GB RSS, so running it alongside the Phase-5 `auto`-default no-worse fit (a SEPARATE gate under `XPCSJAX_RUN_AB_PARITY=1`) in one process OOM-kills the child (returncode `-9`). Never widen this child scope back to the whole module. When run, these do a **live fit against the upstream `homodyne` package** and read its configs/datasets at **hardcoded absolute paths outside the repo** (e.g. `/home/wei/Documents/Projects/data/C020/...`), so they pass only on a maintainer machine that has both. Setting `XPCSJAX_RUN_CHARACTERIZATION=1` directly in CI still fails loudly with "homodyne not importable" / "registered config path is dead" (see the CI-facts memory); the availability gate avoids that by skipping when the prereqs are missing instead of depending on the env var being unset. The test module imports only stdlib + numpy at top level, but that does **not** mean the comparison runs without upstream: the gated test bodies pull in `homodyne` and the local datasets at runtime. `make run-example` (`scripts/generate_homodyne_baselines.py`) likewise needs the upstream packages — install with `uv pip install -e /path/to/homodyne` (and `/path/to/heterodyne` when its generator lands). Neither upstream package is declared in `pyproject.toml`; deliberate — xpcsjax is a JAX-native rewrite and shouldn't pull its predecessors into normal installs.
