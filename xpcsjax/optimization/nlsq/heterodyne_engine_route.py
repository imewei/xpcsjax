...
def fit_two_component_via_engine(
    ...
    # succeeded, else hard-"poor" — mirrors
    # heterodyne_result_builder.py::build_hybrid_streaming_result exactly
    # (its own `if success: classify_quality_flag(...) else: "poor"` gate).
    # A numerically-converged solve that lands on a poor solution (high
    # reduced chi2) must not be reported as "good", but neither should a
    # solve that outright failed (res.success is False) be graded on chi2
    # alone — chi2 can look deceptively low mid-failure.
    from xpcsjax.optimization.nlsq.validation import classify_quality_flag

    quality_flag: QualityFlag = (
        classify_quality_flag(reduced_chi2=reduced_chi2) if res.success else "poor"
    )
