...
def _atomic_json_dump(obj: Any, path: Path) -> None:
    ...
    leaves no truncated artifact behind under the real filename. The temp name is
    generated by ``mkstemp`` (unique + collision-safe) rather than a fixed
    ``path.name + ".tmp"``, since concurrent fit workers (``XPCSJAX_FIT_CONCURRENCY``)
    can otherwise share an output dir and race on the same temp filename.
    """
...
def save_nlsq_npz_file(
    ...
    # Atomic write, same rationale as _atomic_json_dump (unique mkstemp name to
    # avoid concurrent-fit-worker collisions on a fixed ".tmp" suffix). The temp
    # file is opened in binary mode and the handle passed to np.savez_compressed,
    # because numpy appends ".npz" to a *path* argument that lacks the extension
    # — which would write to the wrong name and defeat the temp-then-rename.
    npz_fd, npz_tmp_name = tempfile.mkstemp(
        dir=npz_file.parent, prefix=npz_file.name + ".", suffix=".tmp"
    )
    npz_tmp = Path(npz_tmp_name)
    try:
        with os.fdopen(npz_fd, "wb") as fh:
