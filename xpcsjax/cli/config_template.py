"""YAML template logic for xpcsjax configuration generation.

Provides the mode-to-template map, template path resolution, config generation
from templates, template display, config validation, the interactive prompt
helper, and the interactive config builder.

This module is imported by :mod:`xpcsjax.cli.config_generator`, which re-exports
the public symbols as a facade so existing callers are unaffected.
"""
