...
class DatashaderRenderer:
    ...
    >>> img = renderer.rasterize_heatmap(c2_data.T, t1_coords, t2_coords)
...
def plot_c2_heatmap_fast(
    ...
    *,
    ...
    # c2[t1_idx, t2_idx]: rows=t1, cols=t2. Render with x=t1, y=t2 to match the
    # matplotlib backend (imshow(c2.T, origin="lower") => x-axis=t1, y-axis=t2),
    # so both backends label the same surface identically (rasterize_heatmap's
    # own docstring example uses this x_coords=t1, y_coords=t2 convention).
    ...
    try:
        ...
        ax.imshow(img_array, extent=extent, origin="lower", aspect="auto")
        ax.set_box_aspect(1)
...
def plot_c2_comparison_fast(
    ...
    *,
    ...
    # Render with x=t1, y=t2 (transposed c2) to match the matplotlib backend.
    ...
    try:
        ...
        axes[0].imshow(img_exp_arr, extent=extent, origin="lower", aspect="auto")
        ...
        axes[1].imshow(img_fit_arr, extent=extent, origin="lower", aspect="auto")
        ...
        axes[2].imshow(img_res_arr, extent=extent, origin="lower", aspect="auto")
